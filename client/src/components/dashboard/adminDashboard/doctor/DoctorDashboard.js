...
const DoctorDashboard = () => {
  ...
  const handleUpdateDoctor = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:5000/api/doctors/${selectedDoctorId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        setDoctors(doctors.map((doctor) => (doctor._id === selectedDoctorId ? updatedDoctor : doctor)));
              ...
              {doctors
                ...
                .map((doctor) => (
                  <tr key={doctor._id} className="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    <td className="py-2 px-3 text-xs">{doctor.specialization}</td>
                    <td className="py-2 px-3 text-xs">{doctor.contact}</td>
                    <td className="py-2 px-3 text-xs">{doctor.status}</td>
                    <td className="py-2 px-3 text-xs">LKR{doctor.consultantFee}</td>
                    <td className="py-2 px-3 text-xs">
                      {doctor.visibilityStartDate
                        ? new Date(doctor.visibilityStartDate).toLocaleDateString()
                        : 'N/A'}- {doctor.visibilityEndDate
                          ? new Date(doctor.visibilityEndDate).toLocaleDateString()
                          : 'N/A'}
                    </td>
                    <td className="py-2 px-3 text-xs">{doctor.visibilityStartTime || 'N/A'}-{doctor.visibilityEndTime || 'N/A'}</td>
                    <td className="py-2 px-3 text-xs">
                      <button
  ...
  <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
    <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-3xl">
      ...
      <form onSubmit={handleAddDoctor} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Name Field */}
        <div>
          <label htmlFor="doctorName" className="block mb-1 text-sm font-medium">Name</label>
          <input
            id="doctorName"
            type="text"
            placeholder="Name"
        ...
        {/* Contact Field */}
        <div>
          <label htmlFor="contact" className="block mb-1 text-sm font-medium">Contact</label>
          <input
            id="contact"
            type="text"
            placeholder="Contact"
        ...
        <div>
          <label htmlFor="consultantFee" className="block mb-1 text-sm font-medium">Consultant Fee</label>
          <input
            id="consultantFee"
            type="number"
            ...
            onChange={(e) => setNewDoctor({ ...newDoctor, consultantFee: e.target.value })}
            required
          />
        </div>
        ...
        <div>
          <label htmlFor="visibilityStartDate" className="block mb-1 text-sm font-medium">Visibility Start Date</label>
          <input
            id="visibilityStartDate"
            type="date"
            className="block w-full mb-2 p-2 border rounded-md text-xs"
            value={newDoctor.visibilityStartDate}
            onChange={(e) => setNewDoctor({ ...newDoctor, visibilityStartDate: e.target.value })}
            required
          />
        </div>
        ...
        <div>
          <label htmlFor="visibilityEndDate" className="block mb-1 text-sm font-medium">Visibility End Date</label>
          <input
            id="visibilityEndDate"
            type="date"
            className="block w-full mb-2 p-2 border rounded-md text-xs"
            value={newDoctor.visibilityEndDate}
            onChange={(e) => setNewDoctor({ ...newDoctor, visibilityEndDate: e.target.value })}
            required
          />
        </div>
        ...
        <div>
          <label htmlFor="visibilityStartTime" className="block mb-1 text-sm font-medium">Visibility Start Time</label>
          <input
            id="visibilityStartTime"
            type="time"
            className="block w-full mb-2 p-2 border rounded-md text-xs"
            value={newDoctor.visibilityStartTime}
            onChange={(e) => setNewDoctor({ ...newDoctor, visibilityStartTime: e.target.value })}
            required
          />
        </div>
        ...
        <div>
          <label htmlFor="visibilityEndTime" className="block mb-1 text-sm font-medium">Visibility End Time</label>
          <input
            id="visibilityEndTime"
            type="time"
            className="block w-full mb-2 p-2 border rounded-md text-xs"
            value={newDoctor.visibilityEndTime}
            onChange={(e) => setNewDoctor({ ...newDoctor, visibilityEndTime: e.target.value })}
            required
          />
        </div>
        ...
        <div className="flex justify-end gap-4 col-span-2">
          ...
          <button type="submit" className="bg-blue-500 text-white py-2 px-4 rounded-md text-xs">
            Add Doctor
          </button>
        </div>
      </form>
...
{isEditingDoctor && (
  <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
    <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
      ...
      <form onSubmit={handleUpdateDoctor} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
